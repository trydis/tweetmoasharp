...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		/// </remarks>
		[Test]
		public void Can_stream_from_public_stream()
		{
			const int maxStreamEvents = 5;
			...
			service.StreamFilter((streamEvent, response) =>
			{
				if (streamEvent is TwitterUserStreamEnd)
				{
					block.Set();
				...
				if (response.StatusCode == 0)
				{
					...
					if (streamEvent is TwitterUserStreamStatus)
					{
						...
						Assert.IsNotNull(tweet.RawSource);
						Assert.IsNotNull(tweet.User.ScreenName);
						System.Diagnostics.Debug.WriteLine("Tweet: " + tweet.Text);
					}
			...
			new FilterStreamOptions()
			{
				Track = new string[] { "Twitter" }
			});
